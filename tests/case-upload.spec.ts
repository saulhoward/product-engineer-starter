...
test("submit files and open case", async ({ page }) => {
  ...
  await expect(
    page.getByText("Anterior has received your case..."),
  ).toBeVisible();
  await expect(
    page.getByText("Florence is processing your case..."),
  ...
  await page
    .getByRole("button", { name: /Choose one of the following/i })
    .first()
    .click();
  await expect(
    page.getByText("Suggest trying Medial Branch Block"),
  ).toBeVisible();
});
